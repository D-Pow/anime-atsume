...
module.exports = {
    ...
    plugins: [
        ...
        [
            ...
            {
                cwd: [ 'packagejson' ],
                alias: Object.entries(tsconfig.compilerOptions.paths).reduce((obj, [ key, vals ]) => {
                    // Resolve any alias path matches to be relative from the root dir, then remove any double-slashes
                    // e.g. { '@': 'src', '/': '.' } => { '@': './src/', '/': './' }
                    const alias = key.replace(/\*/g, '').replace(/\/\//g, '/');
                    const aliasRelPaths = vals.map(val => {
                        val = val.replace(/\*/g, '').replace(/\/\//g, '/');

                        const aliasRelPath = `./${path.relative('.', val)}/`.replace(/\/\//g, '/');

                        return aliasRelPath;
                    });

                    obj[alias] = aliasRelPaths;
